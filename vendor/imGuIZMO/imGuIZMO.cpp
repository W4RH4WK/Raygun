...
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char* label, glm::vec4& axis_angle, float size, [[maybe_unused]] const int mode)
{
    imguiGizmo g;
    g.modeSettings(mode & ~g.modeDual);
...
bool gizmo3D(const char* label, glm::vec3& dir, float size, [[maybe_unused]] const int mode)
{
    imguiGizmo g;
...
inline glm::vec3 &adjustSpotCyl(glm::vec3 &coord)
{
    ...
    coord.x = (coord.x*.075f - 2.f +( halfCylMinusCone - halfCylMinusCone*.075f)); //cyl begin where cone end
    return coord;
...
bool imguiGizmo::drawFunc(const char* label, float size)
{
    ...
    glm::vec3 resizeAxes( ((drawMode&modeDual) && (axesResizeFactor.x>.75)) ? glm::vec3(.75f,axesResizeFactor.y, axesResizeFactor.z) : axesResizeFactor);
    ...
    const float halfSquareSize = innerSquareSize*.5f;
    ...
    auto drawComponent = [&] (const int idx, const glm::quat &q, ptrFunc func)
    {
        ...
        for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm+idx)->begin(); itVtx != ptrVtx->end(); ) { 
            ...
            for(int h=0; h<3; h++) {
                ...
                col[h] = addLightEffect(glm::vec3(directionColor.x, directionColor.y, directionColor.z), norm.z, coord.z>0 ? coord.z : coord.z*.5f);
            }
            addTriangle();
        }
